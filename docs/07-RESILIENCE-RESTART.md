...
## Докачка для соцсетей (JobCall/ServiceCall)

**Проблема:** Синк оборвался на середине (rate limit, таймаут, падение). Нужно продолжить, не начиная заново.

**Решение:**

- **Курсоры** хранятся в `ServiceCall.cursor`.
- **Рестарт:** новый JobCall или продолжение по `parent_service_call_id`; cursor из последнего успешного ServiceCall.
- **Шаги:** при повторном проходе проверяют `ext_id` + `platform` — если данные уже есть и актуальны, пропускают или делают upsert (если `updated_at` новее).
